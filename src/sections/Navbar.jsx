...
const Navbar = () => {
  ...
  return (
    <>
            ...
            <div className="flex flex-wrap flex-col md:flex-row gap-x-2">
              {socials.map((social, index) => (
                <a
                  ...
                  href={social.href}
                  target="_blank"
                  className="text-sm tracking-wider leading-loose hover:text-white transition-colors duration-300"
                >
                  {"[ "}
                  {social.name}
                  {" ]"}
      ...
      {/* HAMBURGER ICON */}
      <div
        className="fixed z-50 flex flex-col items-center justify-center gap-1 transition-all duration-300 bg-black rounded-full cursor-pointer h-14 w-14 md:w-20 md:h-20 top-4 right-10"
        onClick={toggleMenu}
        style={
          showBurger
